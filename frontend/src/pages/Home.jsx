...
function Home({ search }) {
  ...
  return (
    <div className={`${hasActiveSearch ? "pt-24 space-y-12" : "space-y-24"} pb-20`}>
      {/* Hero Section */}
      {!hasActiveSearch && heroImageUrl && (
        <section className="relative h-screen min-h-[700px] flex items-center overflow-hidden bg-neutral-dark">
          <div className="absolute inset-0 z-0">
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-r from-neutral-dark/80 via-neutral-dark/40 to-transparent" />
          </div>
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-6 w-full text-white space-y-8">
            <div className="space-y-4 max-w-2xl animate-in fade-in slide-in-from-left duration-1000">
              <h2 className="text-secondary font-bold tracking-[0.2em] uppercase text-sm">Crafted for Excellence</h2>
              <h1 className="text-6xl md:text-8xl font-heading font-bold leading-[1.1]">
            ...
            <div className="flex gap-4 animate-in fade-in slide-in-from-bottom duration-1000 delay-300">
              ...
              <Button size="lg" variant="outline" className="text-white border-white hover:bg-white hover:text-neutral-dark">
                Custom Orders
              </Button>
            </div>
          </div>
      ...
      {/* Featured Benefits */}
      {!hasActiveSearch && (
        <section className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-12 mt-12">
          {[
      ...
      {/* Products Section */}
      <section id="collections" className="max-w-7xl mx-auto px-6 space-y-16 scroll-mt-24">
        {hasActiveSearch ? (
          ...
          groupedBySection.map((section, idx) => (
            <div key={section.name} className="space-y-10 animate-in fade-in duration-700">
              <div className="flex justify-between items-end border-b border-neutral-dark/10 pb-6">
                <div className="space-y-2">
                  <span className="text-secondary font-bold text-xs uppercase tracking-widest">Collection {idx + 1}</span>
                  <h2 className="text-4xl font-heading font-bold">{section.name}</h2>
                </div>
      ...
      {!hasActiveSearch && (
        <section className="max-w-7xl mx-auto px-6">
          <div className="bg-primary p-12 md:p-20 text-center text-white space-y-8 rounded-sm relative overflow-hidden">
            <div className="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full -translate-y-1/2 translate-x-1/2 blur-3xl" />
            <div className="relative z-10 space-y-4 max-w-2xl mx-auto">
              <h2 className="text-4xl md:text-5xl font-heading font-bold">Bring Your Vision to Life</h2>
              ...
              <div className="pt-6">
                <Button size="lg" variant="secondary" className="px-12">Contact for Custom Quote</Button>
              </div>
            </div>
          </div>
        </section>
